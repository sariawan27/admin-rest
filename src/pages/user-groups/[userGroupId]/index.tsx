...
import UserGroupDetails from "@components/user-group/user-group-detail";
...
export default function UserGroupDetailsPage() {
  ...
  const {
    ...
    defaultValues: { order_status: data?.user?.status?.id ?? "" },
  });
  ...
  return (
    <>
      ...
      {loading ? null : (
        <UserGroupDetails permissions={dataPermissions} loadingPermissions={loadingPermissions} />
      )}
...
UserGroupDetailsPage.Layout = Layout;
