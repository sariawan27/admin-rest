...
import AddProductMenu from "@components/product/add-product-menu";
import { getAuthCredentials } from "@utils/auth-utils";

export default function ProductsPage() {
  const { userId, userGroup } = getAuthCredentials()
  const [searchTerm, setSearchTerm] = useState("");
  const [type, setType] = useState("");
  const [category, setCategory] = useState("");
  const [user, setUser] = useState(userGroup[0] === 'superuser' ? "" : userId);
      ...
      <Card className="flex flex-col mb-8">
        <div className="w-full flex flex-col md:flex-row items-center">
          ...
          <div className="w-full md:w-3/4 flex flex-col items-center ms-auto">
            <Search onSearch={handleSearch} isPlaceholder={t("form:input-placeholder-sku-search")} />
          </div>
          ...
          {/* <LinkButton
            ...
            className="h-12 md:ms-6 w-full md:w-auto md:ms-5 mt-5 md:mt-0"
          >
        ...
        >
          <div className="flex flex-col md:flex-row md:items-center mt-5 md:mt-8 border-t border-gray-200 pt-5 md:pt-8 w-full">
            <CategoryTypeFilter
              ...
              onCategoryFilter={({ productCategoryId }) => {
                setCategory(productCategoryId);
              }}
              handleRemoveCategoryFilter={() => { setCategory("") }}
              handleRemoveUserFilter={() => { setUser("") }}
            />
          </div>
        </div>
      </Card>
