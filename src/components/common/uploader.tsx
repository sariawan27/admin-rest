...
export default function Uploader({ onChange, value, multiple }: any) {
  ...
  const handleDelete = (image: string) => {
    const images = files.map((file) => { return { ...file, isDeleted: true } });
    // const images = files.filter((file) => file.productImagePath !== image);
      ...
      return (
        ...
        >
          <div className="flex items-center justify-center min-w-0 w-16 h-16 overflow-hidden">
            <img src={getStorageUrl + file.productImagePath} />
          </div>
          {multiple ? (
            <button
              ...
              onClick={() => handleDelete(file.productImagePath)}
            >
  ...
  useEffect(
    () => () => {
      ...
      files.forEach((file: any) => URL.revokeObjectURL(file.productImagePath));
    },
    [files]
  );
  return (
